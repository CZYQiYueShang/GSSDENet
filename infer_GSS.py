...
# parser.add_argument('--ckpt_name', type=str, default='ConvNeXtB-200')
...
def get_model(network: str,
    ...
    if network == 'GSSDENet-S':
        model = GSSDENet_S(backbone_path=backbone, skip_top=skip_top, device=device)
    else:
    ...
    if os.path.exists(trained_model_path):
        ...
        # print([k for k in model_dict.keys() if (k not in trained_dict)])
        # print([k for k in trained_dict if (k not in model_dict.keys())])
