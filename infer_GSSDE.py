...
def get_model(network: str,
    ...
    if network == 'GSSDENet':
        model = GSSDENet(backbone_path=backbone, skip_top=skip_top, max_depth=max_depth, device=device)
    else:
    ...
    with torch.no_grad():
        ...
        for step, image_name in enumerate(infer_bar):
            ...
            if network == 'GSSDENet':
                ...
                output_depth = predicts[-1] * 1000.0
                output_depth = F.interpolate(output_depth, size=(h, w), mode='bicubic', align_corners=True)
