...
class BoundaryLoss(nn.Module):
    def __init__(self,
        ...
        self.bce_loss_fn = BCELoss(reduction=bce_reduction)
        # self.iou_loss_fn = IoULoss()
        self.dice_loss_fn = DiceLoss(smooth=dice_smooth, reduction=dice_reduction)

    def forward(self,
                inputs: Tensor,
                targets: Tensor) -> Tensor:
        boundary_loss = self.bce_loss_fn(inputs, targets) + self.dice_loss_fn(inputs, targets)
        return boundary_loss


# overall loss for GSD
class GSDLoss(nn.Module):
    def __init__(self,
                 weight: Tuple[float, ...] = (1, 1, 1, 1, 3, 2),
    ...
    def forward(self,
        ...
        if self.boundary_map is False:
            for i in range(len(inputs)):
                loss_list.append(self.context_loss_fn(inputs[i], targets) * self.weight[i])
        else:
            for i in range(4):
                loss_list.append(self.context_loss_fn(inputs[i], targets[0]) * self.weight[i])
            # valid = torch.ones_like(targets[1])
            # for i in range(4, 9):
            #     loss_list.append(self.boundary_loss_fn(inputs[i], targets[1]) * self.weight[i])

            loss_list.append(self.boundary_loss_fn(inputs[-2], targets[1]) * self.weight[-2])
            # loss_list.append(self.context_loss_fn(inputs[-2], targets[2]) * self.weight[-2])
...
# overall loss for GSDE
class GSDELoss(nn.Module):
    def __init__(self,
                 weight: Tuple[float, ...] = (1, 1, 1, 1, 2),
                 variance_focus: float = 0.85,
                 log_depth_error: bool = True,
                 min_depth: float = 0.2,
                 max_depth: float = 10.0) -> None:
        super(GSDELoss, self).__init__()
        self.weight = weight
    ...
    def forward(self,
                inputs: Tuple[Tensor],
                targets: Tensor) -> Tuple[Tensor, List[Tensor]]:
        loss_list: List[Tensor] = []
        for i in range(len(inputs)):
            loss_list.append(self.depth_loss_fn(inputs[i], targets) * self.weight[i])
        total_loss = sum(loss_list)
        return total_loss, loss_list


class GSDDELoss(nn.Module):
    def __init__(self,
                 weight: Tuple[float, ...] = (1, 1, 1, 1, 2, 1, 1, 1, 1, 2),
                 boundary_map: bool = True,
                 bce_reduction: str = 'mean',
                 dice_smooth: float = 1,
    ...
    def forward(self,
                inputs: Tuple[Tensor],
                targets: Tuple[Tensor]) -> Tuple[Tensor, List[Tensor]]:
        loss_list: List[Tensor] = []
        for i in range(4):
            loss_list.append(self.context_loss_fn(inputs[i], targets[0]) * self.weight[i])
        if self.boundary_map is True:
            # valid = torch.ones_like(targets[1])
            loss_list.append(self.boundary_loss_fn(inputs[4], targets[1]) * self.weight[4])
            loss_list.append(self.context_loss_fn(inputs[5], targets[0]) * self.weight[5])
            for i in range(6, 11):
                loss_list.append(self.depth_loss_fn(inputs[i], targets[2]) * self.weight[i])
        else:
            loss_list.append(self.context_loss_fn(inputs[4], targets[0]) * self.weight[4])
            for i in range(5, 10):
                loss_list.append(self.depth_loss_fn(inputs[i], targets[1]) * self.weight[i])
