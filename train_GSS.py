...
from network.GSSDENet_S import GSSDENet_S
from loss import GSDLoss
from optimizer import SGD, PolyScheduler
...
# network
parser.add_argument('--network', type=str, default='GSSDENet-S')
...
# loss function
parser.add_argument('--loss_weight', type=Tuple[float], default=(1, 1, 1, 1, 3, 2))
...
def get_data_loader(network: str,
    ...
    train_loader = GSD.get_data_loader(network, os.path.join(dataset_root, 'train'), batch_size=batch_size,
                                       ...
                                       remove_edge=remove_edge, save_boundary=save_boundary, is_train=True, drop_last=False)
...
def get_model(network: str,
    ...
    if network == 'GSSDENet-S':
        model = GSSDENet_S(backbone_path=backbone_path, skip_top=skip_top, device=device)
    else:
...
def get_loss_function(network: str,
    ...
    if network == 'GSSDENet-S':
        loss_fn = GSDLoss(weight=loss_weight, boundary_map=boundary_map, dice_smooth=dice_smooth)
    else:
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            for loss in loss_list:
                loss_log += '%.4f, ' % loss
            loss_log = loss_log[:-2] + ']'
            train_bar.desc = "train epoch[%d/%d] loss: %s  learning_rate: %.6f" % \
                             (epoch + 1, epochs, loss_log, optim.param_groups[0]['lr'])
            break
...
def main() -> None:
    ...
    model = get_model(args.network, args.backbone_path, args.skip_top, args.device, args.pretrained_model)
